        ...
        for temp in participant:
            if temp.mobile_no1 == mobile_no or temp.mobile_no2 == mobile_no:
                ...
                if temp.event_flag == 0 :
                    ...
                    event = "Clash"
                body = "Dear Participant,\n\n\t\tGreeting from team PISB!\n\t\tYou have participated in Credenz'16 in event " + str(event) +".Slot booking for the same has started at url:_____________.\n\t\tSlot booking is compulsory to participate in the event.\n\t\tYour login credentials are:\n\t\tUsername         : "+str(temp.user.username)+"\n\t\tPassword         : "+str(temp.password)+"\n\t\tMobile NO        : Mobile number given while registration.\n\t\tRegistration No  : Registration number is same as receipt number. If in case you didn't have receipt number then your receipt number will be 9999.\n\n\t\tInsructions:\n\t\t\t1) Mobile number of any one of the team member is valid.\n\t\t\t2) If participated in the team of two, then also one slot will be booked.\n\t\t\t3) Slots are available for round 1 only on 9th and 10th of September.\n\t\t\t4) Slots are of half an hour each.\n\t\t\t5) Once booked slot cannot be cancelled or changed.\n\t\t\t6) If case of any problem for the same contact us."
...
def add_data_manually(request):         #complete
    if request.method == "POST" and request.user.username == "pisb_credenz"  and request.user.is_authenticated() :
        ...
        random_password=random_password_generate()
        temp=User.objects.create_user(username=username,password=random_password)
        UserData.objects.create(registration_id = request.POST['registration_id'],name1 = str(request.POST['name1']),name2 = str(request.POST['name2']),email1 = str(request.POST['email1']),email2 = str(request.POST['email2']),mobile_no1=str(request.POST['mobile_no1']),mobile_no2=str(request.POST['mobile_no2']),event_flag = event,password = random_password, user=temp)
